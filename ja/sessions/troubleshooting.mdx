...
<Accordion title="ドロワーにリモート承認が一時停止中と表示される" icon="pause">
  ...
  スイッチはコンピュータごとに 1 つで、すべてのエージェントで共有されます —
...
<Accordion title="行が古いままか重複している" icon="broom">
  ...
  [理由](/ja/sessions/opencode#reset-needs-opencode-closed)。
</Accordion>
...
セッションが実際に動いているのに `Last event sent: never` と出る場合は、何も拾えていないという意味です。Claude Code ではフックを読み込ませるために再起動してください。Codex では `/hooks` を実行します。OpenCode ではプラグインを読み込ませるために再起動してください。

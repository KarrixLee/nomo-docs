...
six are just `/nomo-pair`, `/nomo-status`, `/nomo-approvals`, `/nomo-reset`,
`/nomo-unpair` and `/nomo-update`. Nothing to prefix, and no menu to find them
under.
...
`/nomo-update` is OpenCode's alone. Claude Code and Codex update through their
own hosts; OpenCode has no host to ask, so the command ships with the plugin.

It reports before it acts — `Already on 2.1.22 — nothing to do.`, or the version
it's about to fetch — then pulls and asks for the same restart everything else
on this page asks for. What it does and when it refuses is in
[Commands](/sessions/commands#update).

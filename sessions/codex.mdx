...
<Tabs>
  <Tab title="Codex CLI" icon="terminal">
    ...
    `bridge available` means you're set — questions reach your phone with
    their options, and you tap one.

    `status-only` means nothing is listening. Start the daemon, then start
    Codex. Order matters: a session that opened first won't pick the daemon up.
  </Tab>

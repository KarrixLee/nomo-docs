...
icon: "code"
keywords: ["Nomo", "Claude Code", "Codex", "iPhone", "session", "computer", "Dynamic Island", "permission", "pairing", "Using Codex"]
---
...
<Note>
  There is no separate Codex pairing. Every agent reads the same config on that
...
<Tabs>
  <Tab title="Codex CLI" icon="terminal">
    ...
    Then confirm, in the `Plan` row of Codex's own section:

    ```bash
    $nomo-status
    ```

    `questions are answerable from your phone` means you're set — questions
    reach your phone with their options, and you tap one.

    `questions can only be answered here` means nothing is listening. Start the
  ...
  <Tab title="Codex desktop app" icon="window">
    **Questions from desktop-app sessions can't be answered on your phone.**
    ...
    no socket to join. There's no flag or setting that changes that, so the `Plan`
    row will keep saying `questions can only be answered here` no matter what you
    start beforehand.
